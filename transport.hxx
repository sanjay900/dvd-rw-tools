...
public:
	...
	unsigned char &operator[](size_t i)
	{
		if (i == 0)
		{
			memset(cmd, 0xff, sizeof(cmd)), memset(&_sense, 0, sizeof(_sense));
			cgc.quiet = 1;
			cgc.sense = &_sense.s;
#ifdef SG_IO
			if (use_sg_io)
...
public:
	...
	int umount(int f = -1)
	{
		...
		while ((i = getmntent(fp, &mb)) != -1)
		{
			...
			if (stat(mb.mnt_special, &msb) < 0)
				continue; // also corrupted?
			if (msb.st_rdev == fsb.st_rdev)
			{
				if (_dev_unmount(mb.mnt_special))
					break;
				{
					struct utsname uts;
				...
				if ((pid = fork()) == (pid_t)-1)
					break;
				if (pid == 0)
					execl("/usr/sbin/umount", "umount", mb.mnt_mountp, (void *)NULL);
				while (1)
				{
					rpid = waitpid(pid, &rval, 0);
					if (rpid == (pid_t)-1)
...
public:
	...
	int transport(Direction dir = NONE, void *buf = NULL, size_t sz = 0)
	{
		...
		case SCTL_INVALID_REQUEST:
			err = "SCTL_INVALID_REQUEST";
			break;
		case SCTL_SELECT_TIMEOUT:
			err = "SCTL_SELECT_TIMEOUT";
			break;
		case SCTL_INCOMPLETE:
			err = "SCTL_INCOMPLETE";
			break;
		case SCTL_POWERFAIL:
			err = "SCTL_POWERFAIL";
...
public:
	...
	int associate(const char *file, const struct stat *ref = NULL)
	{
		...
		if (attr_get(file, "_devname", hw_path, &hw_len, 0))
			return 0;
		if (hw_len >= sizeof(hw_path))
			hw_len = sizeof(hw_path) - 1; // paranoia
		hw_path[hw_len] = '\0';
	...
	int transport(Direction dir = NONE, void *buf = NULL, size_t sz = 0)
	{
		...
		while (retries--)
		{
			if (ioctl(fd, DS_ENTER, &req) < 0)
				return -1;
			if (req.ds_status == STA_GOOD)
				return 0;

			if (req.ds_ret == DSRT_NOSEL)
				continue;
	...
	int umount(int f = -1)
	{
		...
		{
			if (attr_getf(f, "_devname", hw_path, &hw_len, 0))
				return -1;
			if (hw_len >= sizeof(hw_path))
				hw_len = sizeof(hw_path) - 1; // paranoia
			hw_path[hw_len] = '\0';
		...
		while ((mb = getmntent(fp)) != NULL)
		{
			...
			if (msb.st_rdev == fsb.st_rdev)
			{
				...
				if ((pid = fork()) == (pid_t)-1)
					break;
				if (pid == 0)
					execl("/sbin/umount", "umount", mb->mnt_dir, (void *)NULL);
				while (1)
				{
					rpid = waitpid(pid, &rval, 0);
					if (rpid == (pid_t)-1)
...
static int handle_events(Scsi_Command &cmd)
{
	...
	while (events)
	{
		...
		{
		case 0:
			return ret; // No [supported] events
		case 1:
			ret |= 1 << 1; // Operational Change
			if ((descr & 0xFFFF) < 3)
				goto read_profile;
		start_unit:
			if (!started)
		...
		read_profile:
			...
			break;
		case 2:
			ret |= 1 << 2;	  // Power Management
			if (event[5] > 1) // State is other than Active
				goto start_unit;
			break;
		case 3:
			ret |= 1 << 3;
			break; // External Request
		case 4:
			ret |= 1 << 4;	  // Media
...
// 0xC0 means "Multi-session, no Test Write, Incremental"
{
	...
	if ((track[6] & 0x0F) == 1 || (track[6] & 0x0F) == 0x0F)
		...
		fprintf(stderr, ":-( none Mode 1 track\n"),
			exit(FATAL_START(EMEDIUMTYPE));
